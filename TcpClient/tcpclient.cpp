...
{
    if(!OpeWidget::getInstance().getBook()->getDownloadStatus())//如果没有处在下载文件状态
    {
        uint uiPDULen = 0;
        ...
        {
            if (0 == strcmp(pdu->caData, CANCEL_USER_NOT_EXIST))
            {
                QMessageBox::information(this, "注销", CANCEL_USER_NOT_EXIST);
            }
            else if (0 == strcmp(pdu->caData, CANCEL_PASSWORD_ERROR))
            {
                QMessageBox::warning(this, "注销", CANCEL_PASSWORD_ERROR);
            }
            else if (0 == strcmp(pdu->caData, CANCEL_OK))
            {
                QMessageBox::warning(this, "注销", CANCEL_OK);
            }
            break;
        }
        ...
        case ENUM_MSG_TYPE_SEARCH_USR_RESPOND:
        {
            ...
            {
                QMessageBox::critical(this, "搜索", QString("%1:not exist").arg(OpeWidget::getInstance().getFriend()->m_strSearchName));
            }
            else if (0 == strcmp(SEARCH_USR_ONLINE, pdu->caData))
            {
                QMessageBox::critical(this, "搜索", QString("%1:online").arg(OpeWidget::getInstance().getFriend()->m_strSearchName));
            }
            else if (0 == strcmp(SEARCH_USR_OFFLINE, pdu->caData))
            {
                qDebug() << QString("%1:offline").arg(OpeWidget::getInstance().getFriend()->m_strSearchName);
                QMessageBox::critical(this, "搜索", QString("%1:offline").arg(OpeWidget::getInstance().getFriend()->m_strSearchName));
            }
            break;
        }
        case ENUM_MSG_TYPE_ADD_FRIEND_REQUEST:
        {
            ...
            int ret = QMessageBox::information(this, "添加好友", QString("%1 wang to add you as friend, degree?").arg(caName), QMessageBox::Yes, QMessageBox::No);
            PDU* respdu = mkPDU(0);
...
//            qDebug() << QString("agree");
            QMessageBox::information(this, "添加好友", QString("%1已同意您的好友添加请求").arg(caPerName));
            break;
        }
        case ENUM_MSG_TYPE_ADD_FRIEND_REFUSE:
        {
            char caPerName[32] = {'\0'};
            memcpy(caPerName, pdu->caData, 32);
//            qDebug() << QString("refuse");
            QMessageBox::information(this, "添加好友", QString("%1拒绝了您的好友添加请求").arg(caPerName));
            break;
        }
        ...
        case ENUM_MSG_TYPE_PRIVATE_CHAT_REQUEST:
        {
            if (PrivateChat::getInstance().isHidden())
            {
                ...
                PrivateChat::getInstance().setChatName(caSendName); //如果是作为接收信息方打开的privateChat框，需要设置双方名字
            }
        ...
        case ENUM_MSG_TYPE_FLUSH_DIR_RESPOND:
        {
            ...
            if (!strEnterDir.isEmpty()) //如果进入的文件夹非空，则更新当前文件夹
            {
                m_strCurPath = m_strCurPath + "/" + strEnterDir;
                OpeWidget::getInstance().getBook()->clearEnterDir();//清空进入的文件夹
            }
//            qDebug() << m_strCurPath;
            break;
        }
...
//            qDebug() << pdu->caData;
            ...
            {
                OpeWidget::getInstance().getBook()->setDownloadStatus(true);//设置进入下载文件状态
...
void TcpClient::on_pushButton_cancel_clicked()
{
    QString strName = ui->lineEdit_name->text();
    QString strPwd = ui->lineEdit_pwd->text();
    if (!strName.isEmpty() && !strPwd.isEmpty())
